...
void BinarySearchTree<Key, Value>::insert(const std::pair<const Key, Value> &keyValuePair)
{
    ...
    root_ = new Node<Key, Value>(keyValuePair.first, keyValuePair.second, node);
}
...
void BinarySearchTree<Key, Value>::remove(const Key& key)
{
    ...
    if (right != nullptr && node->getParent()->getRight() == node) {
...
template<class Key, class Value>
Node<Key, Value>* BinarySearchTree<Key, Value>::getSmallestNode(Node<Key, Value>* node) {
    Node<Key, Value>* smallest = node;
